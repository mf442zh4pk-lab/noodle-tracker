...
function whoCheckedMeal(comp, users, tid, mealIndex, ds) {
  return Object.entries(users || {})
    .filter(([uid]) => comp?.[mealKey(tid, mealIndex, ds, uid)])
    .map(([uid, u]) => ({ uid, ...u }));
}
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${phase.color}30`, background: `${phase.color}08`, padding: "8px 11px 10px" }}>
          {meals.map((label, mealIdx) => {
            const myMeal = !!comp[mealKey(task.id, mealIdx, dateStr, userId)];
            const checkers = whoCheckedMeal(comp, users, task.id, mealIdx, dateStr);
            const anyDone = checkers.length > 0;
            return (
              <div key={mealIdx} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 9px", marginBottom: mealIdx < meals.length - 1 ? 5 : 0, background: anyDone ? `${phase.color}20` : "rgba(255,255,255,0.7)", borderRadius: 8, border: `1px solid ${anyDone ? phase.color : "#e8ddd0"}` }}>
                <button onClick={() => onToggleMeal(task.id, mealIdx, dateStr)} style={{ width: 20, height: 20, borderRadius: 4, flexShrink: 0, border: `2px solid ${myMeal ? userColor : "#c4b09a"}`, background: myMeal ? userColor : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#fff" }}>{myMeal ? "v" : ""}</button>
        ...
        <div style={{ padding: "20px 16px", maxWidth: 500, margin: "0 auto" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "18px", border: "1.5px solid #e8ddd0" }}>
            ...
            {editDogName ? (
              ...
              <button onClick={() => setEditDogName(true)} style={{ background: "#f0e8d8", border: "1.5px solid #e8ddd0", borderRadius: 10, padding: "9px 16px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 13, color: "#7a5c3a" }}>edit Change puppy name</button>
            )}
          </div>
          <div style={{ height: 32 }} />
        </div>
