          ...
          {meals.map((label, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 9px", marginBottom: i < meals.length - 1 ? 5 : 0, background: anyDone ? `${phase.color}20` : "rgba(255,255,255,0.7)", borderRadius: 8, border: `1px solid ${anyDone ? phase.color : "#e8ddd0"}` }}>
                <button onClick={() => onToggleMeal(task.id, i, dateStr)} style={{ width: 20, height: 20, borderRadius: 4, flexShrink: 0, border: `2px solid ${myMeal ? userColor : "#c4b09a"}`, background: myMeal ? userColor : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#fff" }}>{myMeal ? "v" : ""}</button>
